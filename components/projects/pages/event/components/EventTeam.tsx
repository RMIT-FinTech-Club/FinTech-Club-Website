...
const EventTeam: React.FC<EventTeamProps> = ({ sectionValue }) => {
	...
	const nextMember = () =>
		setCurrentMember((prev) => Math.min(prev + 1, team.length - 1));
	...
	useEffect(() => {
		const interval = setInterval(() => {
			if (
				...
				if (!sliderReversed.current)
					currentMemberRef.current === team.length - 1
						? (sliderReversed.current = true)
						: nextMember();
				else
					currentMemberRef.current === 0
						? (sliderReversed.current = false)
						: prevMember();
	...
	const handleTouchMove = (e: React.TouchEvent) => {
		if (isDragging.current && startPosition.current !== undefined)
			currentPosition.current = e.touches[0].clientX;
		if (currentPosition.current && startPosition.current)
			setMovePosition(startPosition.current - currentPosition.current);
	};
	...
	const handleTouchEnd = () => {
		...
		if (
			...
			if (movedBy > 50 && currentMemberRef.current < team.length - 1)
				nextMember();
	...
	const btnClasses =
		"flex justify-center items-center w-[15vw] aspect-square rounded-full mt-[5vw] mx-[7vw] border border-solid";
	...
	return (
		<div className="flex flex-col justify-center items-center px-[5vw] pt-[10vw] md:pb-[5vw] pb-[10vw]">
			...
			>
				{team.map((member, index) => (
					<a key={index} href={member.link}>
						<div
							style={{
								backgroundColor: member.bgc,
								transform: isMobile
									? `translateX(calc(-${90 * currentMember}vw - ${movePosition * 1.5}px))`
									: "none",
							}}
			...
			<div className="flex md:hidden justify-between">
				...
				>
					<svg
						className="w-[7vw] aspect-square"
						fill={currentMember === 0 ? "#0D1742" : "#FFF"}
