...
const ImageForm6: React.FC<ImageForm6Props> = ({ images = [] }) => {
	...
	return (
		<div className="md:w-[30vw] md:mb-0 mb-[10vw] w-[90vw] aspect-[35/28] relative">
			...
			<div
				className={`${imgClasses} top-[10%] left-1/2 translate-x-[-50%] w-[30%] h-[60%] rounded-[10px]`}
				style={{ backgroundImage: `url(${images[1]})` }}
			></div>
			<div
				className={`${imgClasses} top-0 left-0 w-[40%] h-[80%]`}
				style={{ backgroundImage: `url(${images[0]})` }}
			></div>
			<div
				className={`${imgClasses} ${smallImgClasses} top-0 right-0`}
				style={{ backgroundImage: `url(${images[2]})` }}
			></div>
			<div
				className={`${imgClasses} ${smallImgClasses} top-1/2 translate-y-[-50%] right-0`}
				style={{ backgroundImage: `url(${images[3]})` }}
			></div>
			<div
				className={`${imgClasses} ${smallImgClasses} left-[20%] bottom-[-5%]`}
