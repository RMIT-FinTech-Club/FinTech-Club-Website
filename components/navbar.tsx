...
import React, { useState, useEffect, ButtonHTMLAttributes, use, useRef } from "react";
import { Menu, X, CaretDown } from "tabler-icons-react"; // Including Tabler icons for menu controls
import { MotionConfig, Transition, Variant, Variants, motion, useAnimate } from "framer-motion";
            ...
            <div className="flex justify-between items-center max-w-6xl mx-auto px-4 w-full">
                ...
                >
                    {siteConfig.navItems.map((item) => (
                        <motion.li variants={navBarItemVariants} key={item.href} className="mb-6">
                            <Link
...
const AnimatedHamburger = ({ stroke = 2, size = 24 }: AnimatedHamburgerProps) => {
    ...
    useEffect(() => {
        async function animateBars() {
            if (isOpen) {
                // Closing animation
                animateTopBar(topBarScope.current, { top: '50%' }, {
                    duration: 0.2,
                    type: 'tween',
                    ease: 'easeInOut',
                })
                await animateBottomBar(bottomBarScope.current, { top: '50%' }, {
                    duration: 0.2,
                    type: 'tween',
                    ease: 'easeInOut',
                })
                ...
                // Spinning
                animateTopBar(topBarScope.current, { rotate: 90 }, {
                    duration: 1.4,
                    type: 'spring',
                    bounce: 0.4,
                })
                animateContainerBar(containerBarScope.current, { rotate: 135 }, {
                    duration: 1.4,
                    type: 'spring',
                    bounce: 0.4,
                })
                ...
                // Spin back
                animateContainerBar(containerBarScope.current, { rotate: -0 }, {
                    duration: 0.8,
                    type: 'spring',
                    bounce: 0.2,
                })
                await animateTopBar(topBarScope.current, { rotate: -0 }, {
                    duration: 0.8,
                    type: 'spring',
                    bounce: 0.2,
                })
                ...
                // Opening animation
                animateTopBar(topBarScope.current, { top: '0%' }, {
                    duration: 0.2,
                    type: 'tween',
                    ease: 'easeInOut',
                })
                animateBottomBar(bottomBarScope.current, { top: '100%' }, {
                    duration: 0.2,
                    type: 'tween',
                    ease: 'easeInOut',
                })
    ...
    return (
        <motion.button ref={containerBarScope} style={containerStyles} onClick={() => setIsOpen(!isOpen)} >
            <motion.div ref={topBarScope} style={barStyles} className='top-0' ></motion.div>
            <motion.div style={barStyles} ></motion.div>
            <motion.div ref={bottomBarScope} style={barStyles} className='top-full' ></motion.div>
        </motion.button>
    )
}
