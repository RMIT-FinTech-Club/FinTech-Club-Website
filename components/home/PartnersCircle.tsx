...
export default function PartnersCircle({
	...
	return (
		<nav>
			{isMobile ? (
				<div className="absolute h-[40vh] w-[34vw] border-[0.5vw] border-[#000] rounded-[50%] top-[50%] translate-y-[-52%] left-[-20vw]">
					{items.map((item, index) => (
						<div
							...
							className={clsx(
								...
								"flex justify-center items-center h-[80px] w-[100px] text-[3vh] text-white bg-center bg-no-repeat bg-contain",
								item.bg
							)}
							onClick={() => {
								if (animating) return;
						...
						>
							<div
								className="aspect-square w-1/5 bg-contain bg-no-repeat bg-center"
								style={{ backgroundImage: `url(${item.icon})` }}
							></div>
							{[2, 3, 4].includes(index) && (
								<div
									className={`${styles.pattern} h-full w-[98px] absolute top-0 left-0 rounded-[50%] bg-center bg-no-repeat bg-contain`}
