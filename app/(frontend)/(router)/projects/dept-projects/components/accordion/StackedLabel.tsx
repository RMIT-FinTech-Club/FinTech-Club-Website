...
export default function StackedLabel({
  ...
  if (isHR) {
    ...
    return (
      <span
        className={`${container} relative h-[37rem] flex-row items-stretch justify-between leading-none uppercase tracking-[0.08em] font-bold md:max-lg:gap-[0.25rem] ${poppins.className}`}
        aria-hidden="true"
      >
          ...
          <span className="relative flex flex-col justify-start items-center h-full gap-[0.45rem] md:max-lg:gap-[0.3rem]">
            ...
            <span className="stacked-word-human flex flex-col items-center bg-[#2C305F] z-[10] pb-3">
              {firstWord.map((ch, i) => (
                <span
                  key={`human-${i}`}
                  className="text-[clamp(1.5rem,0.9vw+0.95rem,2rem)] md:max-lg:text-[clamp(1.05rem,0.45vw+0.9rem,1.25rem)] py-1"
                >
                  {ch}
                </span>
              ))}
