...
const ProjectTeam: React.FC<{ teamMembers: TeamMember[] }> = ({ teamMembers }) => {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 pt-8">
        {teamMembers.map((member, index) => (
          <div key={index} className="flex flex-col items-center">
            <img className="w-32 h-32 rounded-full object-cover transform transition-transform duration-300 p-[6px] border-[1px] border-ft-primary-yellow" src={member.imageUrl} alt={member.memberName} />
            <div className="text-center mt-4">
              <h3 className="text-lg font-semibold pb-2">{member.memberName}</h3>
              <div className='w-[20%] h-[4px] bg-ft-primary-yellow mx-auto' />
              <p className="text-gray-500">{member.memberRole}</p>
            </div>
          </div>
        ))}
      </div>
